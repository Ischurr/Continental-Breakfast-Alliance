...
- **Rankings page collapsible posts**: each rankings article shows only the intro paragraph by default; a "+ See more" / "− See less" toggle expands/collapses the rest. Implemented in `app/rankings/RankingsClient.tsx` via `expandedIds` Set state + `extractIntroParagraph()` / `extractRestContent()` helpers (handle both HTML and legacy markup formats). Toggle hidden for single-paragraph articles.
